...
transported matrix eigenvalues, remain explicit typed inputs.
-/
...
/-- Exact MMP seam used downstream: a context-separated source adapter must
supply the positive product roots and the strict log-mesh conclusion. -/
structure MMPLogMeshInput (p : ℝ → ℝ) (d : ℕ) where
  roots : Fin d → ℝ
  strict_log_mesh : StrictLogMesh roots
  roots_are_zeros : ∀ i, p (roots i) = 0

theorem MMPLogMeshInput.hasDistinctPositiveRoots
    {p : ℝ → ℝ} {d : ℕ} (I : MMPLogMeshInput p d) :
    HasDistinctPositiveRoots p d :=
