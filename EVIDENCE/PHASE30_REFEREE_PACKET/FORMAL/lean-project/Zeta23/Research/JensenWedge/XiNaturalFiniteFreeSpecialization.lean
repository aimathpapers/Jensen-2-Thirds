...
normalization and specialization work is performed in Lean. -/
structure XiNaturalClassicalRootInputs
  ...
  mmp : MMPLogMeshInput (xiNaturalComparisonFunction n d L y) d
...
/-- MMP supplies exactly the simple positive comparison roots; the
paper-specific polynomial identification is no longer part of the external
input. -/
...
theorem XiNaturalClassicalRootInputs.comparison_root_product_interval
  ...
  · have hroot :
      ...
      simpa only [xiNaturalComparisonFunction] using I.mmp.roots_are_zeros i
    rw [xiNaturalComparisonPolynomial_eq_finiteFree hn hL hL12 hy d] at hroot
    exact hroot
