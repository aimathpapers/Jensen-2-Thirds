...
phase16_dir="$(cd "$(dirname "$0")" && pwd)"
workspace_root="$(cd "$phase16_dir/../.." && pwd)"
...
(
  ...
  env ELAN_HOME="$elan_root" "$lake_bin" env lean "$phase16_dir/Phase16Axioms.lean"
...
echo "Phase 16 verification PASS"
echo "  recurrence algebra and maximum contradiction kernel-checked"
echo "  fixed-constant radius proof recorded at paper level"
echo "  self-contained sixth-order stability and independent review remain"
