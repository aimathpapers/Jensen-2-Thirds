...
def trust_boundary_check() -> None:
    ...
    required_types = (
        ...
        "MMPFiniteFreeLogMeshInput",
    )
    for required in required_types:
        if required not in body:
            raise SystemExit(f"missing typed literature input: {required}")
    if "xiNaturalComparisonFunction" in body:
        raise SystemExit("MMP field still assumes roots of final xi comparison")
    for required in (
        "jacobiFactorPolynomial",
        "residualParameterA",
        "residualParameterB",
        "residualParameterC",
        "residualParameterD",
        ...
        if required not in body:
            raise SystemExit(f"MMP input is not tied to concrete factors: {required}")
