...
| T11 | Positive simple finite-free model | Section 12; Appendix G, especially the explicit MMP specialization | exact `_3F_2` coefficient identity; factor-level `MMPFiniteFreeLogMeshInput`; Lean transport from the two concrete Jacobi factors to `xiNaturalComparisonFunction`; every convention adapter | source audit | the general MMP v3 real-root and logarithmic-mesh theorems remain typed external inputs | amber |
| T12 | Root and critical-point localization | Section 12; Appendix G | reciprocal MSS conversion with explicit positive-lower-endpoint guards, factor-domain certificates, product interval, complete root-multiset argument, logarithmic-derivative signs, and critical localization | exact checks, guarded-call regression, and source audit | MSS Theorem 1.6 plus the T10/T11 literature inputs | amber |
...
| T18 | Main two-thirds wedge | Theorem 1.1; Sections 16--19; Appendix J | `riemannXiJensen_twoThirds_global_headline_exactly` performs the all-`n` cutoff split and proves exactly `d` distinct negative roots; the degree-`d` polynomial object and no-`d+1` theorem close the root count; the xi multiplier certificate and actual-polynomial identification remain concrete | exact CAS, Arb examples, dependency ledger, guarded MSS regression, multiline axiom mutation, and fresh replay | classical Jacobi/MMP/MSS results remain explicitly typed theorem inputs | green, conditional on amber dependencies |
