...
def validate(text: str) -> None:
    checks = {
        ...
        "factor-level MMP seam": "structure MMPFiniteFreeLogMeshInput",
        "first factor hypothesis": "first_factor : HasDistinctPositiveRoots p.eval d",
        "second factor hypothesis": "second_factor : HasDistinctPositiveRoots q.eval d",
        "convolution output": "roots_are_zeros : ∀ i, (finiteFreeAscending d p q).eval",
        "positive distinct output": "theorem MMPFiniteFreeLogMeshInput.hasDistinctPositiveRoots",
    }
...
def main() -> None:
    ...
    mutations = {
        ...
        "MMP seam hidden": (
            ...
            "structure AssumedMMPFiniteFreeLogMeshInput",
        ),
        "first factor hypotheses hidden": (
            "first_factor : HasDistinctPositiveRoots p.eval d",
            "first_factor : True",
        ),
        "convolution disconnected": (
            "roots_are_zeros : ∀ i, (finiteFreeAscending d p q).eval",
            "roots_are_zeros : ∀ i, p.eval",
        ),
    }
