...
MSS finite-free interval input, and one factor-level
`MMPFiniteFreeLogMeshInput`. The MMP record is attached to the two displayed
Jacobi factor polynomials and includes their positive-root and degree
hypotheses; it no longer assumes that `xiNaturalComparisonFunction` itself
has the desired roots. Lean proves the concrete terminating `_3F_2` equals
that finite-free convolution and transports the roots across the equality.
Lean checks every project-specific normalization, convention adapter,
interval use, and downstream implication, but it does not re-prove the
general third-party Jacobi/MMP/MSS theorems from first principles.

The finite-cutoff absorption arithmetic is now kernel checked by
`not_twoThirdsWedge_finiteCutoffAbsorption`: below the analytic cutoff, the
wedge antecedent is empty after enlarging the final constant.
